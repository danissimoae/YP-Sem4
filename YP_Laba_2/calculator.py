...
class Calculator:
    def __init__(self, start_x: int, end_x: int, amount_of_points: int):
        if start_x >= end_x:
            raise Exception('Неверно задан диапазон')

        if amount_of_points <= 0:
            raise Exception('Неверно задано количество точек')
    ...
    def __diff_multiply(self, operand_1: str, operand_2: str):
        expr = f"{self.__differentiative(operand_1)} {operand_2} * {operand_1} {self.__differentiative(operand_2)} * +"

        return expr

    def __diff_div(self, operand_1: str, operand_2: str):
        expr = f"{self.__differentiative(operand_1)} {operand_2} * {operand_1} {self.__differentiative(operand_2)} * - {operand_2} 2 ^ /"

        return expr

    def __diff_add(self, operand_1: str, operand_2: str):
        expr = f"{self.__differentiative(operand_1)} {self.__differentiative(operand_2)} +"
    ...
    def __diff_pow(self, operand_1: str, operand_2: str):
        if 'x' in operand_2:
            raise Exception('Введенная функция не дифференцируема')

        expr = f"{operand_2} {operand_1} {operand_2} 1 - ^ * {self.__differentiative(operand_1)} *"

        return expr

    def __diff_un_min(self, operand_1: str):
        expr = f"{self.__differentiative(operand_1)} ~"

        return expr

    def __diff_sin(self, operand_1: str):
        expr = f"{operand_1} cos {self.__differentiative(operand_1)} *"

        return expr

    def __diff_cos(self, operand_1: str):
        expr = f"{operand_1} sin ~ {self.__differentiative(operand_1)} *"

        return expr

    def calc_rpn(self, expression_in: str):
        if expression_in.count('(') != expression_in.count(')'):
            raise Exception('Неверная формула')
        ...
        self.__derivative_of_an_expression_rpn = self.__differentiative(self.__expression_rpn)
    ...
    def __compute(self, expression_rpn: str, x: float = None):
        ...
        for elem in expr:
            if elem == 'x':
                if not x:
                    raise Exception("Отсутствует значение для параметра x")
                operand = float(x)
            ...
            match elem:
                ...
                case "/":
                    try:
                        ...
                        raise ZeroDivisionError('Попытка деления на нуль')
    ...
    def __differentiative(self, expression_rpn: str):
        expr = expression_rpn.split()
        ...
        if elem in self.__bin_operators:
            ...
            match elem:
                case '*':
                    derivative_of_an_expression_rpn = self.__diff_multiply(operand_1, operand_2)
                case '/':
